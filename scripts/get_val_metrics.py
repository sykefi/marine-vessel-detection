...
def get_val_results(
    ...
    end2end:bool=False, # Use end2end detection for models that support it
):
    ...
    with tempfile.TemporaryDirectory() as sd:
        res = m.val(device='cuda', project=None, save_dir=sd, plots=False, half=half, verbose=False, 
                    ...
                    compile=True, show_conf=False, show_labels=False, visualize=False, end2end=end2end)
    ...
    if end2end:
        outfn = 'val_results_best_end2end.csv'
    else:
        outfn= 'val_results_best.csv'
    df.to_csv(model_dir/outfn, index=False)
    ...
    df = pd.DataFrame(outdict)
    if end2end:
        outfn = 'val_results_last_end2end.csv'
    else:
        outfn= 'val_results_last.csv'
    df.to_csv(model_dir/outfn, index=False)