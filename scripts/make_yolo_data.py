...
@call_parse
def make_yolo_data(
    annotation_path:Path, # Path to the folder containing the annotated ships
    ...
    dataset_format:str='box' # Format of annotations, either box, mask or obb
):
    "Convert `gpkg` annotations to yolo-format data split into `n_folds` spatial folds"

    if dataset_format not in ['box', 'obb', 'mask']:
        print(f'{dataset_format} not supported, supported formats are "box", "obb" and "mask"')
    ...
    datasets = [annotation_path/f'{d}.gpkg' for d in train_datasets]

    # Tile data
    for ds in datasets:
        tile = ds.stem.split('_')[0]
        print(f'Processing {tile}')
        os.makedirs(tempdir/ds.stem, exist_ok=True)
        mosaics = [mosaic_path/tile/f for f in os.listdir(mosaic_path/tile) if f.endswith('tif')]
        for m in (mosaics):
            ...
            tiler.tile_raster(m)
            tiler.tile_vector(str(ds), gpkg_layer=m.stem.split('T')[0].split('_')[-1],
                              min_area_pct=0)
            ...
            for row in fold_grid.itertuples():
                ...
                for im in tqdm(os.listdir(im_dir)):
                    ...
                    if os.path.exists(vector_dir/im.replace('tif', 'geojson')):
                        ...
                        gdf = gdf.clip(clip_poly, keep_geom_type=True)
    ...
    for fold in folds:
        shp_to_yolo(outpath/fold/'images', outpath/fold/'vectors', outpath/fold, 
                    'id', cats, ann_format=dataset_format, min_bbox_area=0)
