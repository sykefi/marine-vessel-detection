...
def evaluate(
    ...
    for t in tiles:
        ...
        result_files = [f for f in os.listdir(result_dir/t) if f.endswith(('geojson', 'gpkg'))] 
        for tstep in tsteps:
            matching_res = [f for f in result_files if tstep in f][0]  
            for ix, i in enumerate(ious):
                ...
                match ann_type:
                    case 'box':
                        targs['geometry'] = targs.geometry.apply(lambda row: box(*row.bounds))
                    case 'obb':
                ...
                results = ap_per_class(tps_for_yolo, 
                                       confs_for_yolo, 
                                       pred_classes,
                                       target_classes, 
                                       plot=False,
    ...
    data_type = 'cleaned' if filter_preds else 'raw'

    f1 = 2 * (outdict['precision'] * outdict['recall'])/(outdict['precision'] + outdict['recall'])

    print(f"""
    Results for {data_type} data: 
        Precision: {outdict['precision']:.4f}
        Recall: {outdict['recall']:.4f}
        F1-score: {f1:.4f}
        mAP50: {outdict['mAP50']:.4f}
        mAP50-95: {outdict['mAP50-95']:.4f}
        True positives: {outdict['tp']}
        False positives: {outdict['fp']}
        False negatives: {outdict['fn']}
    """)
