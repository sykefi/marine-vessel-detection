...
def make_test_report(
    ...
    for model in model_arcs:
        if not os.path.exists(metric_dir/model/f'{eval_fname}_end2end.json'): continue
        res = pd.read_json(metric_dir/model/f'{eval_fname}_end2end.json', orient='index').T
        res.rename(columns={'mAP50': 'ap50', 'mAP50-95': 'ap'}, inplace=True)
        res['model_type'] = model.split('_')[0]
        res['train_type'] = model.split('_')[-1]
    ...
    outdf = outdf[['model_type', 'train_type', 'pred_type', 'precision', 'recall', 'f1',
                   'ap50', 'ap', 'tp', 'fp', 'fn']]
