...
def clean_stationary_targets(gdf:gpd.GeoDataFrame, preset:str=None, keep_fps:bool=False) -> gpd.GeoDataFrame:
    ...
    # Note: Limits the predictions to Finnish waters
    ...
    if rem > 0:
        if os.path.exists(PATH_TO_FISHERIES):
            ...
            print('Data for fisheries not found, skipping')
...
@call_parse
def main(yolo_weights:str, # Path to ultralytics model weights to use
    ...
    "Run ultralytics detection model with sahi for the full S2_L1C -tile and save predictions to `outpath`"
    ...
    # Initialize model
    det_model = UltralyticsDetectionModel(model_path=yolo_weights,
                                          device=device,
    ...
    # Georeference predictions. Doesn't really need image but rather 
    # the affine object `im.transform` and crs `im.crs`
